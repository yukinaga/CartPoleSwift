...
//        recordStatus()
        ...
        if pole.zRotation < -CGFloat.pi/4 || pole.zRotation > CGFloat.pi/4 {
            agent.updateQTable(reward: -1, nextState: getState())
            ...
            agent.updateQTable(reward: 0, nextState: getState())
        }
        
        // Action
        let action = agent.getAction(c_state: getState(), episode: episode)
